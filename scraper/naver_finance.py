...
LIST_URL = "https://finance.naver.com/news/news_list.naver?mode=LSS2D&section_id=101&section_id2=262"
...
def get_headline_list(pages=1):
    """네이버 금융 '해외증시' 뉴스 목록에서 (제목, 링크) 목록을 가져온다.

    pages를 늘리면 더 과거(더 아래) 기사까지 가져온다.
    """
    results = []
    for page in range(1, pages + 1):
        response = requests.get(LIST_URL, params={"page": page}, headers=HEADERS)
...
def get_us_market_headlines(pages=5):
    ...
    return filter_us_market_news(get_headline_list(pages=pages))
